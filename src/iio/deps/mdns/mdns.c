...
ipv4_address_to_string(char* buffer, size_t capacity, const struct sockaddr_in* addr,
	...
	if (ret == 0) {
		if (addr->sin_port != 0)
			len = _snprintf(buffer, capacity, "%s:%s", host, service);
		else
			len = _snprintf(buffer, capacity, "%s", host);
	}
	if (len >= (int)capacity)
		len = (int)capacity - 1;
	mdns_string_t str;
...
service_mdns(const char* hostname, const char* service_name, int service_port) {
	...
	_snprintf(service_instance_buffer, sizeof(service_instance_buffer) - 1, "%.*s.%.*s",
	...
	_snprintf(qualified_hostname_buffer, sizeof(qualified_hostname_buffer) - 1, "%.*s.local.",
